...
/**
 * Borrows from https://www.npmjs.com/package/elo-rating
 */
@singleton()
export class EloCalculator {
     ...
     */
    public ratingDifference(playerRating, opponentRating) {
     ...
     */
    public expected(playerRating, opponentRating) {
     ...
     */
    public calculate(playerRating, opponentRating, playerWin = true, k = 20): { playerRating: number, opponentRating: number } {
        const ratingChange = this.calculateMutation(playerRating, opponentRating, playerWin, k);
        return {
     ...
     * @param {bool} playerWin Flag indicating if player one has won.
     * @param {number} k K Factor. By default 20.
     * @return rating to be exchanged between player and opponent
     */
    public calculateMutation(playerRating, opponentRating, playerWin = true, k = 20): number {
        const playerExpected = this.expected(playerRating, opponentRating);
        return parseInt(String(k * (Number(!!playerWin) - playerExpected)), 10);
    }
}
